...
## retruned in a list. The 4 functions are set() get(),setmatrix(x),and getmatrix().
## <<- sign is used to create a global variable m, to store cached 
## inverse matrix.
...
## matrix is cached by judding (is.null(m)) statment, the cached data m 
## is used without calculation. Otherwise, proceed to compute matrix reverse 
## value by calling solve() function and cache the result in m, by calling tmp$setmatrix(m)
